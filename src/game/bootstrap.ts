...
export const STARTING_TURN_THRESHOLD = 100
...
const STARTING_ACHIEVEMENTS = [
  { id: 'diceRolled', name: 'Dice Rolled', progress: 0, rewardId: 'startingDie' as const },
  { id: 'moneyEarned', name: 'Money Earned', progress: 0, rewardId: 'startingCash' as const },
  {
    id: 'bonusMoneyEarned',
  ...
  { id: 'bonusXpEarned', name: 'Bonus XP Earned', progress: 0, rewardId: 'bonusXpRate' as const },
  {
    id: 'turnPointsEarned',
  ...
  { id: 'multGained', name: 'Mult Gained', progress: 0, rewardId: 'bonusMultRate' as const },
]
...
export function createStartingDie(index = 0, ladderProgress = 0): DieState {
  ...
  return {
    id: `die-${index + 1}`,
    name: index === 0 ? 'Starter Die' : `Die ${index + 1}`,
    prestige,
    faceCap,
    currentFace: null,
    dieMult: 1,
...
export function createInitialRunState(progress: PersistentProgressState): RunState {
  const startingDieReward =
    progress.rewards.find((reward) => reward.rewardId === 'startingDie')?.amount ?? 1
  const startingCash =
    progress.rewards.find((reward) => reward.rewardId === 'startingCash')?.amount ?? 0
  const startingProgress =
    progress.rewards.find((reward) => reward.rewardId === 'startingDieProgress')?.amount ?? 0
