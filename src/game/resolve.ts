...
const trickById = Object.fromEntries(
  MVP_TRICKS.map((trick) => [trick.id, trick]),
) as Record<TrickId, (typeof MVP_TRICKS)[number]>;
...
function createTrickLine(
  ...
  const multiplier = round(
    (trick.baseMultiplier + extraMultiplier) * (mastered ? 2 : 1),
  );
  const baseValue = round(
    groups.reduce((sum, group) => sum + averageBaseValue(group), 0),
  );
  ...
  return {
    ...
    multiplier,
    faces,
    participatingDieIds: groups.flatMap((group) =>
      group.map((die) => die.dieId),
    ),
...
function detectSetFamily(
  ...
  const orderedGroups = [...faceGroups.entries()].sort(
  ...
  if (unlockedTrickIds.includes("fourKind")) {
    ...
    if (match) {
      return [
        createTrickLine("fourKind", [match[1].slice(0, 4)], trickProgress),
      ];
    }
  }
  ...
  if (unlockedTrickIds.includes("threeKind")) {
    ...
    if (match) {
      return [
        createTrickLine("threeKind", [match[1].slice(0, 3)], trickProgress),
      ];
    }
  }
...
function buildBestStreak(faceGroups: Map<number, RolledDie[]>) {
  const uniqueFaces = [...faceGroups.keys()].sort(
  ...
  return (
    streaks.sort((left, right) => {
      ...
      return (
        right.reduce((sum, face) => sum + face, 0) -
        left.reduce((sum, face) => sum + face, 0)
      );
    })[0] ?? []
  );
}
  ...
  return save.progress.achievements.map((achievement) => {
    switch (achievement.id) {
      case "diceRolled":
        return {
          ...achievement,
          progress: round(achievement.progress + save.run.dice.length),
        };
      case "moneyEarned":
        return {
          ...achievement,
          progress: round(achievement.progress + totalReward.money),
        };
      case "bonusMoneyEarned":
        return {
          ...achievement,
          progress: round(achievement.progress + extraReward.money),
        };
      case "bonusXpEarned":
        return {
          ...achievement,
          progress: round(achievement.progress + extraReward.xp),
        };
      case "turnPointsEarned":
        return {
          ...achievement,
          progress: round(achievement.progress + totalReward.turnPoints),
        };
      case "multGained":
        return {
          ...achievement,
...
function buildLogEntries(summary: ResolvedTurnSummary, priorLogLength: number) {
  const entries = [
    {
...
export function resolveTurn(
  ...
  const baseExtraReward = sumRewards(
    rolledDice.map((die) => getExtraReward(die)),
  );
  ...
  const totalReward = addReward(
    addReward(baseReward, trickReward),
    extraReward,
  );
  ...
  return {
    ...
    run: {
      ...
      dice: save.run.dice.map((die) => {
        const rolled = rolledDice.find(
          (candidate) => candidate.dieId === die.id,
        );
      ...
      log: [
        ...buildLogEntries(summary, save.run.log.length),
        ...save.run.log,
      ].slice(0, 12),
    },
  };
